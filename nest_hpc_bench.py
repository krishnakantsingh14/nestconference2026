...
params = {
    # threads per rank — NEST ignores OMP_NUM_THREADS, so set it explicitly,
    # but read FROM the SLURM-exported value so the two never drift apart.
    "num_threads": int(os.environ.get("NEST_NUM_THREADS",
                       os.environ.get("OMP_NUM_THREADS", 1))),
    "scale":       float(os.environ.get("NEST_SCALE",      1.0)),
    "simtime":     float(os.environ.get("NEST_SIMTIME",    1000.0)),
    "presimtime":  float(os.environ.get("NEST_PRESIMTIME", 100.0)),
    "dt":          0.1,
    # "memory" = compute the rate but write NO files (best for timing benchmarks)
    # "ascii"  = write per-VP .dat files (what gave you 128 files)
    "record_spikes": os.environ.get("NEST_RECORD", "memory") != "off",
    "record_to":     os.environ.get("NEST_RECORD", "memory"),
    "path_name":   os.environ.get("NEST_OUTDIR", "."),
    "log_file":    "log",
}
...
#        E_recorder = nest.Create("spike_recorder", params={"record_to": "ascii", "label": recorder_label})
        E_recorder = nest.Create("spike_recorder",
                         params={"record_to": params["record_to"],
